...
function handler_row($str){
    if(strpos($str, '@')===false)
    {
        return $str;
    }else{
        $arr=explode('@', $str);
        $str=$arr[0].'<br>@'.$arr[1].'</br>';
        return $str;
    }
}
 ...
 *
 */
function buildSelect($tableName,$selectName,$valueField,$textFieldName,$selectedValue=''){
    $model=D($tableName);

    $data=$model->field("$valueField,$textFieldName")->select();
